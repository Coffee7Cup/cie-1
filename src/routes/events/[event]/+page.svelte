<script>
	...
	onMount(() => {
		json = fetch(
			`https://raw.githubusercontent.com/coffee7cup/ciewebrepo/main/events/${galleryName.replace(/\s+/g, '')}.json`
		)
			.then((res) => res.json())
...
<PageParent>
	<div class="w-full min-h-screen text-(--text-color) relative z-1">
		<header class="pt-20 pb-10 px-8 lg:px-20 relative overflow-hidden">
			...
			></div>
			<h1 class="text-4xl lg:text-8xl font- Russo-One font-bold relative z-10 leading-tight">
				{galleryName}
			</h1>
		</header>
